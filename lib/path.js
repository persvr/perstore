...
exports.resolve = function(model, id, metadata){
	...
	if(id.indexOf('.') > -1 && (id.indexOf('?') == -1 || id.indexOf('.') < id.indexOf('?'))){
		...
		for(var i = 1; i < parts.length; i++){
			print("i: " + i + " : parts.length : " + parts.length);
			value = value && (value.get ? value.get(decodeURIComponent(parts[i]),(i==parts.length-1)?metadata:undefined) : value[decodeURIComponent(parts[i])]);
		}
		return value;
	}
	if(id === '' || id.match(/\?|\[/)){
