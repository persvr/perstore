...
exports.AdaptiveIndex = function(store, options) {
    /**
     * In to wrap a store with adaptive indexing the underlying store must
    ...
    // add a catchall incase the underlying store changes out from under us
    ...
    function updateStatistics(i) {
        ...
        if (stats[i]) {
            ...
            when(currentIndex, function(response) {
                ...
                if (response) {
                    // TODO confirm the index is right, e.g. unique, collation?
