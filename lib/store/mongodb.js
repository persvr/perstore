...
//dir('LIMIT', options);
	...
	return {
		...
		get: function(id){
			return when(callAsync(collection.findOne, [{id:String(id)}]), function(obj){
				if (obj) delete obj._id;
dir('GOT:', id, obj); 
				return obj;
			});
			/*var path = id.split('.');
			var obj = when(callAsync(collection.findOne, [{id:path.shift()}]), function(obj){
				if (obj) delete obj._id;
			...
			if (directives.overwrite === false) {
				...
				return callAsync(collection.findOne, [search]).then(function(found){
					if (found === undefined) {
						...
						return callAsync(collection.insert, [object]).then(function(obj){
							// .insert() returns array, we need the first element
							return obj && obj[0] && obj[0].id;
						});
					} else {
						// TODO: this error is unhandled and dumps node! May be silently ignore it?!
						throw new PreconditionFailed(object.id + " exists, and can't be overwritten");
					}
				});
			}
			return callAsync(collection.update, [search, object, !directives.overwrite]).then(function(obj){
dir('PUT:', object, obj); 
				return obj && obj.id;
			});
		},
