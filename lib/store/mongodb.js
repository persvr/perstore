...
exports.MongoDB = function(options){
	...
	var ready = defer(); 
	var collection;
	database.open(function(db, err){
		if(err){
			print("Failed to load mongo database " + dbOptions.name + " error " + err);
			ready.reject(err);			
		}
		else{
			db.collection(options.collection, function(err, coll){
				if(err){
					print("Failed to load mongo database collection " + dbOptions.name + " collection " + options.collection + " error " + err);
					ready.reject(err);			
				}else{
					collection = coll;
					ready.resolve();
