...
// patch ObjectID
BSON.ObjectID.createPk = function(oid) {
	// TODO: directly convert oid to hex?
	//return (new BSON.ObjectID(oid)).toHexString();
	return BSON.ObjectID.toHexString(oid);
}
...
exports.MongoDB = function(options){
	...
	return {
		...
		get: function(id){
			return callAsync(collection.find, [{_id:to_id(id)}]).then(function(results){
			...
			function walk(name, terms) {
				...
				terms.forEach(function(term){
					...
					// http://www.mongodb.org/display/DOCS/Advanced+Queries
						...
						if (key == '_id') args = args.map(function(x){return to_id(x);});
