...
function dir(){var sys=require('sys');for(var i=0,l=arguments.length;i<l;i++)sys.debug(sys.inspect(arguments[i]),false,4);}
...
//dir('RANGE', options);
			...
			var totalCountPromise = (options.totalCount !== Infinity) ?
				options.totalCount :
			...
			return callAsync(collection.find, [search, options]).then(function(results){
				...
				return when(resultsPromise, function(results){
					...
					if (fields) {
						...
						if (options.unhash) {
							// FIXME: fields = ['id'] ok. fields = ['id','x'] fails -- returns some function...
							results = jsValues.apply(results, fields);
//							results = when(jsValues.apply(results, fields), function(r){
//dir('VALS:', r);
//								return r;
//							});
						}
