...
*/
			...
			query.forEach(function(term){
				...
				if (term.comparator !== undefined) {
					...
					var exprs = ['lt','lte','gt','gte','ne','in','nin','not','mod','all','size','exists','type','elemMatch'];
					var requires_array = ['in','nin','all','mod'];
					var func;
					var key = term.name;
					var value = term.value;
					if (func=funcs[term.comparator]) {
						if (key instanceof Array)
							key = key.join('.');
						if (func == 'ne' && value instanceof RegExp) {
							func = 'not';
						} else if (func == 'eq') {
							search[key] = value;
							func = undefined;
						}
						if (exprs.indexOf(func) > -1) {
							if (requires_array.indexOf(func) > -1 && !(value instanceof Array))
								value = [value];
							func = '$'+func;
						}
						if (func !== undefined) {
