...
function dir(x){sys.puts(sys.inspect(x));}
...
//dir(['C?:',options, directives]);
			if (directives.range !== undefined) {
				...
				// new limit is the minimum of remainder of shifted range and the range length itself
				if (directives.range.end - directives.range.start < options.limit)
					options.limit = directives.range.end - directives.range.start;
//dir(['C1:',options,directives.range]);
				//directives.range.start = options.skip; // ???
				if (sliceLimit < options.limit)
					options.limit = sliceLimit;
			}
//dir(['C!:',sliceLimit,options]);
