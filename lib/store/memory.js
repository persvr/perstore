...
var Persistent = exports.Persistent = function(options) {
    if (!options.path) throw new Error("No path for store provided");
    var store = Memory(options);

    // set up a memory store and populate with line-separated json
    var buffer;
    try {
        buffer = fs.readFileSync(options.path);
    }
    catch(e) {}
    ...
    var writeStream = fs.createWriteStream(options.path, {flags: "a", encoding: "utf8"});
