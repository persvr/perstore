...
var Persistent = exports.Persistent = function(options) {
    ...
    store.transaction = function() {
        ...
        return {
            commit: function() {
            	require("sys").puts("commiting");
                if(!writeStream){
                	throw new Error("Store was not initialized. Store's setId should be called or it should be included as part of a data model package")
                }
                return when(writeStream, function(writeStream){ 
	                queue.forEach(writeStream.write);
	                queue.length = 0;
                });
            },
            abort: function() {
