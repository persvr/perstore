...
var parseQuery = exports.parseQuery = function(/*String*/query, parameters){
	...
	if(query.replace(/([&\|,])?(([\+\*\-:\w%\._]+)(=[a-z]*=|=|=?<|=?>|!=)([\+\*\$\-:\w%\._,]+)|(([\+\*\\$\-:\w%\._]+)(\(?))|(\()|(\))|(.+))/g,
		 //      <-delim-> <--- name ---------  comparator   -----   value  ---->|<-function/value -- openParan->|<-groupParan->|<-closedParan->|<-illegalCharacter->
		function(t, termDelimiter, expression, name, comparator, value, call, functionOrValue, openParan, groupParan, closedParan, illegalCharacter){
			if(comparator){
				var comparison = {
					type:"comparison", 
					...
					conjunction: termDelimiter
				};
				terms.push(comparison);
			}
			else if(call || groupParan){
				if(openParan || groupParan){
					var children = [];
					if(functionOrValue){
						// a function call
						terms.push({
