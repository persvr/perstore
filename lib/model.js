...
exports.initializeRoot = function(dataModel){
	function setPath(model, path){
		if(typeof model === "object"){
			for(var name in model){
				setPath(model[name], path ? path + '/' + name : name);
			}
		}
		else if(model.setId){
			model.setId(path);
...
exports.ModelsModel = function(models){
	var modelStore = require("./store/memory").Memory({index: models});
