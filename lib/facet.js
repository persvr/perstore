...
function FacetedStore(store, facetClass){
	...
	constructor.query= function(query, directives){
		...
		if(typeof facetClass.query !== "function"){
			...
			return this.wrap(store.query(query), this.transaction);
		}
		return this.wrap(facetClass.query(query), this.transaction);
	};
	...
	constructor.get= function(id){
		...
		if(id.indexOf('.') > -1 && (id.indexOf('?') == -1 || id.indexOf('.') < id.indexOf('?'))){
			...
			for(var i = 1; i < parts.length; i++){
				...
				if(value && value.$ref){
					value = constructor.get(value.$ref);
				}
			}
			return value;
		}
		...
		if(typeof facetClass.get !== "function"){
			...
			return this.wrap(store.get(id), this.transaction);
		}
...
exports.Restrictive = function(appliesTo, schema){
	...
	for(var i in appliesTo){
		if(!facet[i] && i.substring(0,3) == "get"){
			(function(i){
				facet[i] = function(){
					appliesTo[i].apply(appliesTo, arguments);  
				}
			})(i);
		}
	}
