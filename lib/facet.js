...
function FacetedStore(store, facetClass){
	...
	constructor.checkQuery = function(query){
		...
		if(!lastLimit && maxRange != Infinity){
			throw new RangeError("This user is not allowed to execute a query without a range specified through a Range header or a limit operator in the query");
		}
		if(lastLimit > maxRange){
			throw new RangeError("This user is not allowed to execute a query with a limit of " + lastLimit + " the user has maximum range of " + maxRange);
		}
	...
	constructor.get = function(id, directives){
		...
		function getFromTransaction(id, transaction){
			...
			for(var i = 1; i < parts.length; i++){
				store = store.get(parts[i], directives);
			}
			return store;
		}
		if(id.indexOf('.') > -1 && (id.indexOf('?') == -1 || id.indexOf('.') < id.indexOf('?'))){
			...
			for(var i = 1; i < parts.length; i++){
				...
				if(value && value.$ref){
					value = constructor.get(value.$ref, directives);
				}
			}
			return value;
		}
		...
		if(parts){
			...
			for(var i = 0; i < parts.length; i++){
				var part = parts[i];
				value = part[0] === '.' ? value.get(part.substring(1), directives) : value[part.substring(1)];
			}
			return value;
		}
		if(id.match(/^\(.*\)$/)){
			...
			return all(parts.map(function(part){
				return self.get(part, directives);
		...
		if(typeof facetClass.get !== "function"){
			...
			return this.wrap(store.get(id, directives), this.transaction);
		}
		return this.wrap(facetClass.get(id, directives), this.transaction);
	};
							...
							value: function(directives){
								...
								try{
									if(typeof facetSchema.put === "function"){
										...
										if(typeof sourceClass.put === "function"){
...
exports.Restrictive = function(appliesTo, schema){
	...
	for(var i in appliesTo){
		if(!facet[i] && i.substring(0,3) == "get"){
			(function(i){
				facet[i] = function(){
					return appliesTo[i].apply(appliesTo, arguments);  
				}
			})(i);
		}
	}
