...
function FacetedStore(store, facetClass){
	...
	var maxRange = constructor.maxRange || store.maxRange
		|| 50;

	constructor.checkQuery = function(query){
		var lastSlice;
		var checkOperator = function(operator, checkSlice){
			var name = operator.name;
			if(!allowedOperators[name]){
				throw new AccessError("Query operator " + name + " not allowed for this user");
			}
			if(allowedOperators[name] === "indexed" &&
					!indexedProperties[name === "sort" ? operator.args[0].replace(/^[-\+]/,'') : operator.args[0]]){
				throw new AccessError("Query operator " + name + " not allowed for unindexed property " + operator.args[0] + " for this user");
			}
			operator.args.forEach(function(value){
				if(value && value.name && value.args){
					if(checkSlice && value.name === "slice"){
						lastSlice = value;
					}
					checkOperator(value);
				}
		...
		if(!lastSlice){
			throw new RangeError("This user is not allowed to execute a query without a range specified through a Range header or a slice operator in the query");
		}
		if(!(lastSlice.args[1] - lastSlice.args[0] <= maxRange)){
			throw new RangeError("This user is not allowed to execute a query with the range of " + lastSlice.args[0] + " - " + lastSlice.args[1] + " the user has maximum range of " + maxRange);
		}
	...
	constructor.get= function(id, directives){
		...
		function getFromTransaction(id, transaction){
			...
			for(var i = 1; i < parts.length; i++){
				store = store.get(parts[i]);
			}
			return store;
		}
		if(id.indexOf('.') > -1 && (id.indexOf('?') == -1 || id.indexOf('.') < id.indexOf('?'))){
			var parts = id.split(".");
			var value = this.get(parts[0], directives);
			for(var i = 1; i < parts.length; i++){
				value = value && (value.get ? value.get(parts[i],directives) : value[parts[i]]);
				if(value && value.$ref){
					value = facetClass.get(value.$ref, directives);
				}
			}
			return value;
		}
...
//								sourceClass.get(source.getId ? source.getId() : source.id || source._id).set(name, value);
						...
						put: {
							value: function(object, directives){
								this.set(directives.id, object);
								this.save();
							},
							enumerable: false,
							writable: true
